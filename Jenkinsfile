pipeline {
  ...
  stages {
    stage("Build") {
      steps {
        container("kaniko") {
          sh "/kaniko/executor --context `pwd` --destination vfarcic/jenkins-demo:latest --destination ${REGISTRY_USER}/${PROJECT}:$BRANCH_NAME-${env.BRANCH_NAME.toLowerCase()}"
        }
      }
    }
    stage("Test") {
      ...
      steps {
        container("kustomize") {
          ...
          """
          sh "curl http://${env.BRANCH_NAME.toLowerCase()}$PROJECT.3.124.47.165.nip.io"
